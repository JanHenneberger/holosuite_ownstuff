...
%         endTime = [APSDateNum(end)];
    ...
    if any(choosePlots == 1)
        ...
        if beadsCase < 4
            ...
            for cnt = 1:numel(HOLIMOData)
                %histReal{cnt} = histReal{cnt} * 1.178;
                histRealArray(cnt,:) = histReal{cnt};                
                histRealArrayAll{beadsCase} = histRealArray;
                middle2Array(cnt,:) = middle2{cnt};
            end
            errorbar(middle2Array(1,:),mean(histRealArray,1),std(histRealArray,1,1),'b','LineWidth',2)
...
HOLhist.Num6um = histc(HOLData{4}.pDiam*1e6,histBorder);
HOLhist.Num10um = histc(HOLData{5}.pDiam*1e6,histBorder);
HOLhist.Num18um = histc(HOLData{6}.pDiam*1e6,histBorder);
