...
anParameter.intervallVec = [0 0 0 0 0 10];
...
anParameter.ParInfoFolder = 'Z:\2_HOLOSUITE_Data\CL2014_Blowtest\as-Files_New';
%anParameter.ParInfoFolder = 'Z:\5_ParticleStats\CLACE2013-1-as';
...
%clear dataAll
anParameter.timeStart = datenum([2014 2 26 16 28 0]);
anParameter.timeEnd = datenum([2014 2 26 16 50 0]);
%anParameter.timeStart + datenum([0 0 1 0 0 0]);
%anParameter.asSearch = '2013-02-11';
...
%         %temp.pStats.dateNumPar = ones(1,numel(temp.pStats.dateNumPar))*temp.pStats.dateNumPar(1);
    ...
    dataAll.timeStart = anParameter.timeStart;
    dataAll.timeEnd = anParameter.timeEnd;
    dataAll.dateVecHoloStart = datevec(dataAll.timeStart);
    dataAll.dateVecHoloEnd = datevec(dataAll.timeEnd);
...
if ~isfield('anDataAll','timeStart');
    for cnt = 1:numel(anDataAll.intTimeStart)
        display(['calculate first intervals calculations: ' num2str(cnt) '/' num2str(numel(anDataAll.intTimeStart))]);
...
if ~isfield('anDataAll','measMeanV');
    ...
    for cnt = 1:numel(anDataAll.intTimeStart)
        display(['calculate Sonic intervals: ' num2str(cnt) '/' num2str(numel(anDataAll.intTimeStart))]);
        anDataAll.sonicIndice{cnt} = sort([find(dataSonic.Date_Num >= anDataAll.intTimeStart(cnt),1,'first') ...
            find(dataSonic.Date_Num <= anDataAll.intTimeEnd(cnt),1,'last')]);
        if numel(anDataAll.sonicIndice{cnt}) == 1
            anDataAll.sonicIndice{cnt} = [anDataAll.sonicIndice{cnt} anDataAll.sonicIndice{cnt}]
        end
...
%     for cnt = 1:numel(anDataAll.intTimeStart)
%         display(['calculate Manchester intervals: ' num2str(cnt) '/' num2str(numel(anDataAll.intTimeStart))]);
%         %Data from Gary
%         anDataAll.cdpIndice{cnt} = sort([find(dataManchester.cdp.dateNum>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataManchester.cdp.dateNum<= anDataAll.intTimeEnd(cnt),1,'last')]);
%         anDataAll.pvmIndice{cnt} = sort([find(dataManchester.pvm.dateNum>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataManchester.pvm.dateNum<= anDataAll.intTimeEnd(cnt),1,'last')]);
%         if numel(anDataAll.cdpIndice{cnt}) == 1
%             anDataAll.cdpIndice{cnt} = [anDataAll.cdpIndice{cnt} anDataAll.cdpIndice{cnt}];
%         end
%         if numel(anDataAll.pvmIndice{cnt}) == 1
%             anDataAll.pvmIndice{cnt} = [anDataAll.pvmIndice{cnt} anDataAll.pvmIndice{cnt}];
%         end
...
%         %Data from PSI
%         anDataAll.PSIIndice{cnt} = sort([find(dataPSI.date>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataPSI.date<= anDataAll.intTimeEnd(cnt),1,'last')]);
%          if numel(anDataAll.PSIIndice{cnt}) == 1
%             anDataAll.PSIIndice{cnt} = [anDataAll.PSIIndice{cnt} anDataAll.PSIIndice{cnt}];
%         end
...
%         anDataAll.ManchCDPIndice{cnt} = sort([find(dataManchesterCDP.dateNum>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataManchesterCDP.dateNum<= anDataAll.intTimeEnd(cnt),1,'last')]);
...
%         anDataAll.ManchRotorIndice{cnt} = sort([find(dataManchesterRotor.dateNum>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataManchesterRotor.dateNum<=anDataAll.intTimeEnd(cnt),1,'last')]);
...
%         anDataAll.ManchMetekIndice{cnt} = sort([find(dataManchesterMetek.dateNum>= anDataAll.intTimeStart(cnt),1,'first') ...
%             find(dataManchesterMetek.dateNum<= anDataAll.intTimeEnd(cnt),1,'last')]);
...
if true
    ...
    for cnt= 1:numel(anDataAll.intTimeStart);
        display(['calculate Histograms: ' num2str(cnt) '/' num2str(numel(anDataAll.intTimeStart))]);
        tmp = dataAll.intervall == cnt;
...
save(['CLACE2013-10-New-Blowtest-' anParameter.asSearch '-' num2str(anParameter.intervallVec(6)) 'secStat.mat'],'anDataAll','-v7.3');