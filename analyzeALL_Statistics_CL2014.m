if ~exist('anData','var');  
    tmp  = load('Z:\6_Auswertung\CLACE2014\CLACE2014_Ov__300secStat.mat');
    anData = tmp.anDataAll;
end
close all
...
isValid = anData.TWCountRaw > 4 &  anData.isValidFlow;
oIsValid = ordinal(isValid,{'Valid','notValid'},[1,0]);


%Group by isCloud
isCloud = anData.TWContent > 0.001; %& anData.TWConcentraction > .1;
...
oIceFractionNumber=ordinal(Y,labels,[],edges);
...
oTemperature = mergelevels(oTemperature, {'-5 to -0°C','-10 to -5°C','-15 to -10°C'}, '-15 to -0°C');
...
if 0
    ...
    subplot(2,2,1)
    xData = anData.meteoWindVel(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
    xData(~isfinite(xData)) = nan;
    %yData = anData.TWConcentraction;
    yData = anData.IWContent./anData.TWContent;
    yData = yData(oWindDirection == choseWindDirection & oIsCloud == 'Cloud'  & oIsValid == 'Valid');
    yData(~isfinite(yData)) = 0;
    yData(yData < 0) = 0;
    xBins = 6;
    yBins = 6;
    ...
    subplot(2,2,2)
    xData = anData.measMeanT(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
    xData(~isfinite(xData)) = nan;
    %yData = anData.TWConcentraction;
    yData = anData.IWContent./anData.TWContent;
    yData = yData(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
    ...
    xData = anData.meteoWindVel(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
    xData(~isfinite(xData)) = nan;
    yData = anData.TWConcentraction(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
    yData(~isfinite(yData)) = 0;
    yData(yData < 0) = 0;
    xBins = 6;
    yBins = 12;
...
if 0
    figure(111)
    choseWindDirection = 'North wind';
    choseData = oWindVMeteo(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' ...
        & oIsValid == 'Valid');   
    ...
    stat=tabulate(oIceFraction(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid'));
    
    plot(levels,[stat{:,3}],'k','LineWidth',2.5)
    hold on
    for cnt = 1:numel(labels)
        tmp = oIceFraction(oWindDirection == choseWindDirection & oIsCloud == 'Cloud' & oIsValid == 'Valid');
...
%     end
    ...
    legend([{'All Temp.'},legendString],'Location','NorthWest')
    xlabel('IWC/TWC')
    ylabel('Probability [%]')
    %ylim([-5,105])
    box on
...
if 1
    ...
    frequency = summary(oWindDirection(oIsCloud == 'Cloud' & oIsValid == 'Valid'));
    clear freqString
    for i=1:numel(frequency)
        freqString{i} = [' (n = ' num2str(frequency(i)) ')'];
    end
    ...
    for cnt = 1:numel(labels)
        stat=tabulate(oIceFraction(oWindDirection==labels{cnt} & oIsCloud == 'Cloud' & oIsValid == 'Valid'));
...
if 0
    figure(15)
    clear gcf
    chosenData = oWindDirection == 'South wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid';% | oWindDirection == 'South wind';
    plotGroupBoxplotsWindV(anData, oWindVMeteo, chosenData);
end
if 0
    figure(154)
    clear gcf
    chosenData = oWindDirection == 'North wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid';% | oWindDirection == 'South wind';
    plotGroupBoxplotsWindV(anData, oWindVMeteo, chosenData);
    plotGroupBoxplotsWindVUlrike(anData, oWindVMeteo, chosenData);
end
...
if 0
    figure(7)
    clear gcf
    chosenData = oWindDirection == 'North wind'& oIsCloud == 'Cloud' & oIsValid == 'Valid';
...
if 0
    ...
    hold on
%     choseChase = oIsCloud == 'Cloud' & oIsValid == 'Valid' & oWindDirection == 'North wind' &...
%       ~isundefined(oIceFraction2);
%     choseData = oIceFraction2(choseChase);
%     choseChase = oIsCloud == 'Cloud' & oIsValid == 'Valid' & oWindDirection == 'North wind' &...
%       ~isundefined(oWindVMeteo);
%     choseData = oWindVMeteo(choseChase);
    choseChase = oIsCloud == 'Cloud' & oIsValid == 'Valid' & oWindDirection == 'North wind' &...
      ~isundefined(oTemperature);
    choseData = oTemperature(choseChase);
...
if 1
    ...
    hold on
        ...
        plotData = anData.water.histRealCor(:,oIsCloud == 'Cloud' & oIsValid == 'Valid');
        plotData(~isfinite(plotData)) = nan;
        p1 = plot(anData.Parameter.histBinMiddle, ...
            nanmean(plotData,2), ...
            'LineWidth',2,'Color','k');
...
%     plotData = anData.water.histRealCor(:,oWindDirection == 'North wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid');
%     plotData(~isfinite(plotData)) = nan;
%     p3 = plot(anData.Parameter.histBinMiddle, ...
%         nanmean(plotData,2), ...
%         'LineWidth',2,'Color','r');
%     plotData = anData.ice.histRealCor(:,oWindDirection == 'North wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid');
%     plotData(~isfinite(plotData)) = nan;
...
%     
%     
%     plotData = anData.water.histRealCor(:,oWindDirection == 'South wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid');
%     plotData(~isfinite(plotData)) = nan;
%     p5 = plot(anData.Parameter.histBinMiddle, ...
%         nanmean(plotData,2), ...
%         'LineWidth',2,'Color',[0 0.8 0]);
%     plotData = anData.ice.histRealCor(:,oWindDirection == 'South wind' & oIsCloud == 'Cloud' & oIsValid == 'Valid');
%     plotData(~isfinite(plotData)) = nan;
...
%         'LineWidth',2,'LineStyle','--','Color',[0 0.8 0]);
    ...
    %         'LineWidth',2);
%     h_legend = legend([p3 p5 p4 p6], {'Liquid - North wind', 'Liquid - South wind', ...
%         'Ice - North wind',...
%         'Ice - South wind'}, 'Location', 'NorthEast');
    ...
    hold on
        ...
        plot(anData.Parameter.histBinMiddle, ...
           ...
           (1/6*pi.*anData.Parameter.histBinMiddle.^3), ...
            'LineWidth',2,'Color','k');
        plotData = anData.ice.histRealCor(:,oIsCloud == 'Cloud' & oIsValid == 'Valid');
        plotData(~isfinite(plotData)) = nan;
        plot(anData.Parameter.histBinMiddle, ...
            nanmean(plotData,2)'.*...
           (1/6*pi.*anData.Parameter.histBinMiddle.^3), ...
...
tmpInd = isValid & isCloud & oWindDirection == 'North wind';
