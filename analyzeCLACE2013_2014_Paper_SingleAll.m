...
anParameter.asSearch = '2013-02-07-11-29';
...
%Holo Data input
for cntFile = 1:numel(anParameter.ParInfoFiles)
    clear dataAll
    %load as-files
    fprintf('Merge Particle Statistik File: %04u / %04u\n',cntFile, numel(anParameter.ParInfoFiles) );
    fprintf(anParameter.ParInfoFiles{cntFile});
    temp = load(anParameter.ParInfoFiles{cntFile});
    temp = temp.outFile;
    ...
    %bug removal
    if nanmin(temp.pStats.dateNumPar) <1e4
        temp.pStats.dateNumPar = temp.dateNumHolo(temp.pStats.nHoloAll);
    end
    ...
    if ~isfield('anDataAll','timeStart');
        for cnt = 1:numel(anDataAll.intTimeStart)
            ...
            anDataAll.pDiamOldSizesOldThresh{cnt} = [];
            anDataAll.pDiamOMean{cnt} = [];
            anDataAll.imDiamMean{cnt} = [];
            anDataAll.imDiamMeanOldSizes{cnt} = [];
        
            anDataAll.imEquivDia{cnt} = [];        
                anDataAll.imEquivDiaOldSizes{cnt} =[];
                
            anDataAll.rtDp{cnt} = [];
            anDataAll.imMeanRadii{cnt} = [];
        
            anDataAll.majorAxisLength{cnt} = [];
    ...
    if true
        ...
        parDiam2 = anDataAll.pDiam{cnt}(indIce &...
...
%Total Water calculations
    ...
    save(['CLACE2013-1-Ice-' anParameter.ParInfoFiles{cntFile}(1:end-7) '-' num2str(anParameter.intervallVec(6)) 'secStat.mat'],'anDataAll','-v7.3');
