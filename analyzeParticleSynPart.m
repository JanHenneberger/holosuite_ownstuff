...
%         partBorder = [smallBorder(1:(divideInd-1)) bigBorder];
        ...
        HOLIMOData{cnt}.hist = histogram(HOLIMOData{cnt}.pDiam(HOLIMOData{cnt}.partIsReal)...
            *1000000, HOLIMOData{cnt}.sampleVolumeAll, partBorder);

 
   
        HOLIMO.intAll(cnt) =  sum(HOLIMOData{cnt}.pDiam);
...
if any(choosePlots == 1)
    ...
    handleGraph(1,runNr(cnt)) = plot(HOLIMOData{runNr(cnt)}.middle2, HOLIMOData{runNr(cnt)}.hist./max(HOLIMOData{runNr(cnt)}.hist), '--','Color',plotColor(cnt),'LineWidth',2);

    hGroup(runNr(cnt)) = hggroup;
    
    sum(HOLIMOData{runNr(cnt)}.middle2.*HOLIMOData{runNr(cnt)}.hist)/sum(HOLIMOData{runNr(cnt)}.hist)
